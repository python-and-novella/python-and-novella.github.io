...
除了使用JavaScript读写剪贴板，为了方便查看剪贴板的读写效果，代码中还是额外定义一部分Python代码，不过这部分代码不是必须的：由JavaScript代码发射一个名为“clipboard”的自定义事件，并把读取结果通过事件的`args`属性随事件传递；Python代码中通过响应自定义事件（后面会介绍如何响应自定义事件，这里不展开解释）来接收事件并获取`args`属性，完成剪贴板的读取。
...
说起来，前面那么多示例使用了`ui.run`方法，零星使用过该方法的不少参数，还没有完整介绍过该方法的每个参数。为了避免读者使用其参数时不知用法，本章特地介绍一下该方法的各个参数。
...
- `routes`参数，字典类型（键为子页面的路径，值为子页面构建函数的函数名），表示子页面与具体构建函数的对应关系。
  ...
  注意，子页面的路径`'/'`是必须的，不定义的话，单页面应用会报404错误。另外，子页面的路径表示的是相对于单页面应用所属页面的路径。如果单页面应用挂载在`/index`页面下，那所有子页面的路径在转换为实际路径时，全部添加在`/index`之后。
...
本章为先导内容，不介绍具体控件。从下一章开始，每期介绍一个控件的用法。
...
- `'latex'`，表示启用LaTeX的公式支持，即将“\$”或者“\$\$”包围的LaTeX公式为MathML（HTML格式的公式表达）。
  ...
  注意，该功能需要额外安装`latex2mathml`库。此外，因为LaTeX公式会包含类似转义符号的表达格式，字符串必须为原始字符串（在字符串前加`r`修饰符）。
...
- `'openByDefault'`键（仅限列组），布尔类型，表示列组是否默认展开，默认为`False`。
...
在讲完具体用法之后，这里再做个简单的总结，汇总介绍一些可以合并为一类用法的相关用法，并提供必要的概念解释和一些清晰的示例。
...
- `'children'`键，元素为字典（列定义）的列表，表示该列此时为列组，该键对应的值表示包含的子列。
  ...
  这是定义列组的关键，只有包含该键的列才是列组。

- `'openByDefault'`键，布尔类型，表示列组是否默认展开，默认为`False`。
...
使用编辑时，必须使用的表格定义是`'rowSelection'`键（其他键可参考 https://www.ag-grid.com/javascript-data-grid/grid-options/#reference-selection-rowSelection ）。该键是字典类型，其支持以下键：

- `'mode'`键，字符串类型（仅支持`['singleRow','multiRow']`中的值），表示选择方式（单选、多选）。

- `'enableClickSelection'`键，布尔类型或者字符串类型（仅支持`['enableDeselection','enableSelection']`中的值），表示是否允许通过点击该行的其他位置来选择（或者取消选择）该行。注意，如果该键为启用，则需要同时按住`ctrl`键、`shift`键才能完成多选（按键含义同Excel）。

- `'checkboxes'`键，布尔类型，表示是否显示每行的勾选框。

- `'checkboxLocation'`键，字符串类型（仅支持`['selectionColumn','autoGroupColumn']`中的值），表示勾选框的位置（在独立的列、与分组信息在同一列）。对比效果可以参考 https://www.ag-grid.com/javascript-data-grid/grouping-row-selection/ 。

- `'hideDisabledCheckboxes'`键，布尔类型，表示是否隐藏不可选择行的勾选框。

- `'isRowSelectable'`键，使用字符串表达的JavaScript函数，表示行是否可以选择。该JavaScript函数支持以下位置参数（为了方便记忆，这里命名了参数，但实际使用时不限制参数名）：

  - `node`参数，`IRowNode`类型，为该函数专用的参数。`IRowNode`类型支持的属性可以参考 https://www.ag-grid.com/javascript-data-grid/row-object/ 。
  ...
  ![2026_52_163](nicegui_pro.assets/2026_52_163.png)

- `'copySelectedRows'`键，布尔类型，表示是否复制选中的行而不是当前获得焦点的单元格。注意，该功能与企业版相关，社区版不支持。

- `'enableSelectionWithoutKeys'`键，布尔类型，表示在启用`'enableClickSelection'`键的前提下，是否可以不用同时按住`ctrl`键、`shift`键就能完成多选（按键含义同Excel）。
...
- `'masterSelects'`键，字符串类型（仅支持`['self','detail']`中的值），表示选择主行的实际效果（主行、从行）。注意，该功能与企业版相关，社区版不支持。

除了`'rowSelection'`键，表格定义中还有其他键与选择相关的常用键：

- `selectionColumnDef`键，字典类型，表示行选择列（即启用行选择之后，每行勾选框对应列）的列定义

- `enableCellTextSelection`键，布尔类型，表示是否允许选择单元格内的文字，默认为`False`。

  没错，选择单元格内的文字，也是一种“选择”。

如果想要获取已选择的行，则可以使用下面的方法：
...
注意，下面仅介绍常用定义的含义，其他或者更多定义的具体用法、含义可以参考前面的内容或者官方链接 https://www.ag-grid.com/javascript-data-grid/row-sorting/ 。
...
- `'icons'`键，字典类型，表示表格中的图标。虽然表格默认使用了内部统一的图标，但依然可以通过指定键（键名可参考 https://www.ag-grid.com/javascript-data-grid/custom-icons/#icon-names 中左边的名字）修改其图标。
- `'rowHeight'`键，整数类型，表示所有行统一的行高。
- `'rowStyle'`键，字典类型，表示行的样式。
- `'rowClass'`键，字符串类型或者元素为字符串的列表，表示行的样式类。
- `'rowClassRules'`键，字典类型（键为样式类，值为使用字符串表达的JavaScript函数或者表达式），表示行的样式类。不同于`'rowClass'`键只能设置所有行的样式类，该键可以将符合字典值对应条件的行，设置为字典键同名的样式类。完整用法可参考 https://www.ag-grid.com/javascript-data-grid/row-styles/#reference-styling-rowClassRules 。
...
本节参考文档：https://altair-viz.github.io/user_guide/data.html

前面演示了使用和不使用`pandas`库的示例，可能就有读者不太清楚控件到底支持使用什么样的数据，这里简单介绍一下。

`altair`库的图表对象都有`data`这个参数，用于接收数据，主要支持以下几种数据：

- `pandas.DataFrame`类型的对象。
- 实现`__dataframe__`方法的对象，常见于兼容`pandas`库的其他Python库（比如`polar`库和`pyarrow`库）。
- `altair.Data`类型及其派生类型（比如`UrlData`类、`InlineData`类、`NamedData`类）的对象。
- JSON文件或者CSV文件的网络地址。
