...
- 运行程序。完成前面的步骤之后，直接运行Python文件，并不会显示控件，因为NiceGUI程序需要通过特殊的方法运行。比如示例中的`ui.run`方法就是一种运行程序的方法，视图模型、显示模式不同，该方法使用的参数也有所不同。此外，运行程序的方法也不止这一种，还有与现有FastAPI应用组合运行使用的`ui.run_with`方法。
...
从NiceGUI 3.0.0开始，NiceGUI程序按照是否使用`ui.page`方法可划分为两种视图模型，对应着两种基本结构：

- 脚本模型，不使用`ui.page`方法创建页面，所有控件
...
样式知识基础，CSS以及NiceGUI支持的颜色，设置控件样式的三种方法
