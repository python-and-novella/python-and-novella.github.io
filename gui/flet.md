...
Flet是一个基于Flutter框架的GUI框架，就像基于Qt框架的PySide一样，可以使用Python语言构建桌面端、Web端、移动端程序。Flet的界面思维偏向Web，如果有Web开发基础，使用Flet开发程序简直如鱼得水。当然，受限于官方教程不够系统直观且无中文，中文开发者想要入门有点费劲，因此，笔者将在梳理了官方教程和Flet的使用技巧之后，用中文重新构建入门教程。
...
准备好环境之后，接下来正式开始Flet的开发工作。但在学习基础之前，还需要了解一个开发调试的技巧。



（优化一下措辞）

flet run 可以指定运行哪个文件（可以不是main.py)或者哪个目录下的`main.py`

开发过程中建议使用`flet run -d`运行，可以监视文件或者目录下的文件变化，自动重启程序，让变化实时生效。

flet run -d -r [script]可以监视目录及其子目录下文件变化，自动重启程序，让变化实时生效。



（先认真看一遍官网教程，梳理出中文入门教程的大纲，和内容的学习顺序）

基本结构

Flet程序的基本组成，事件机制，美化等
