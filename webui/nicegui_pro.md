...
- 脚本模式。不使用`ui.page`创建页面、不给`ui.run`的第一位置参数`root`传值的话，所有在全局作用域内创建的控件都属于“主页面”（地址为网站的根路径），这样的构建模式就是脚本模式。此时的“主页面”不是真正意义上的主页面，虽然每个访问者打开的“主页面”内容互相独立，但这种构建模式只支持一个页面，即“主页面”。
