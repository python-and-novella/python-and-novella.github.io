...
功能组件是为了实现特定功能而定义的组件，主要是指那些不好分类的组件。在这些组件中，有几个组件的布局位置接受自定义，因此划分到这个分类。那些布局位置像其他组件一样接受自定义的功能组件，则在后面的自动功能组件一节中单独介绍。
...
相比于WebUI和桌面程序，TUI确实有点简陋，但胜在性能开销更小（表格较大和后台任务较多的情况除外），信息密度更高。不过，TUI程序对于开发者来说，需要考虑的问题比WebUI和桌面程序多一些，各个平台的终端多种多样，指令响应、样式表现也不尽相同。input

在写本教程的期间，考虑到Textual框架对Rich框架的依赖，便同时开始Rich教程的编写。虽然RIch的官方文档很简单，但考虑到没有中文，框架的示例较少且存在潜在的坑，便还是打算写一下，并排期到另一款桌面程序框架教程之前。

至此，Textual的中文入门教程完结，后续根据Textual框架更新情况择机添加更新内容，也可能根据框架的使用情况，增加Textual常见问题解答（含示例代码）。
