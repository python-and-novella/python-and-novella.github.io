...
需要特别注意的是，部分控件的字典类型参数如果有使用`@property`修饰的同名属性，则只能修改参数对应的同名属性，然后让其触发控件的自动刷新。无法通过修改字典类型参数的引用值，然后手动调用刷新方法的方式刷新控件。
...
若字典类型参数没有同名属性或者没有使用`@property`修饰的同名属性，则只能且必须在修改字典类型参数的引用值或者未使用`@property`修饰的同名属性之后，手动调用刷新方法来刷新控件。
...
注意，脚本模式的单页面应用**不支持**自定义404页面，强行使用会导致单页面应用的子路由失效。
...
（下面的概念名词和表达需要仔细斟酌，示例全部改为使用单页面模式的示例）
...
如果脚本模式的单页面应用让子页面显示404页面的话，会有什么样的结果？听起来很难想象，那就用代码寻找答案：
